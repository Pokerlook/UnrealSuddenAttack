...
#include "SA/AbilitySystem/Ability/Locomotion/GA_Prone.h"
...
bool UGA_Prone::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, OUT FGameplayTagContainer* OptionalRelevantTags) const
{
...
void UGA_Prone::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	{
		CrouchToProne(Character);
	}
	else if (OwnerTags.HasTag(SATags.State_Stance_Prone))
	{
		ProneToStand(Character);
	}
	else
	{
		StandToProne(Character);
	}
}
